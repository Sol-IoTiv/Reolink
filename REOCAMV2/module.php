...
class ReolinkV2 extends IPSModuleStrict
{
   ...
   public function GetConfigurationForm(): string
    ...
    /**
     * Push schreiben wie im realen Dump getestet: nur Push.enable ändern.
     *
     * Wichtig:
     * - kein schedule.enable schreiben
     * - keine schedule.table schreiben
     * - dadurch bleibt der Zeitplan vollständig unverändert
     */
    private function apiWritePushPayloads(mixed $value): bool
    {
        $enable = (bool)$value ? 1 : 0;

        // Minimaler Payload für SetPushV20 / SetPush.
        // apiFeatureSet kapselt diesen Knoten automatisch als ['Push' => ...].
        $payload = [
            'channel' => 0,
            'enable'  => $enable,
        ];

        return $this->apiFeatureSet('push', $payload, 'PUSH-SET-ENABLE-ONLY');
    }
    ...
    private function GetMdAlarmNodeForScope(string $topic = 'MD-SCOPE', bool $preferInitial = false): ?array
    {
        ...
        // Für die Erkennungsfläche brauchen wir nur scope.table.
        // Beim Schreiben wird bewusst nur scope gesendet, damit die MdSensitivity
        // nicht durch initial/value-Dummywerte der Firmware überschrieben wird.
        $node = $preferInitial
            ? ($root['initial'][$nodeName] ?? $root['value'][$nodeName] ?? null)
            : ($root['value'][$nodeName] ?? $root['initial'][$nodeName] ?? null);

        if (!is_array($node) || !isset($node['scope']) || !is_array($node['scope'])) {
            return null;
        }
    ...
    {
        $id = @$this->GetIDForIdent('MdDetectionArea');
        if ($id === false) {
            return;
        }
